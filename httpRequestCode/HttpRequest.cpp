...
void Http::hostInit() {
	...
	compareValue["www.91listen.com"] = &Http::listen91;
}
...
void Http::listen91(std::string& postdatastr, std::string& content, std::string& url,\
		std::string& topidId, std::string& hostUin, std::string& uin) {
	std::cout << "listen91" << std::endl;
	std::string uri;
	std::string host;
	findURI(url.c_str(), host, uri, "http://");
	std::map<std::string, std::string>::iterator itpostArg;
	if ((itpostArg = postArg.find(host)) != postArg.end()) {
		...
		std::cout << "return listen91 NULL" << std::endl;
		return;
	}
}
...
int Http::sendRequstCurl(std::string& url, std::string& postdatastr, std::string& cookie, std::string& response) {
	...
	if (curl != NULL) {
		...
		curl_easy_setopt(curl, CURLOPT_TIMEOUT, 30); // 接收超时时间
